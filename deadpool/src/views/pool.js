 ...
 * scripts/field.py carries the same warning.
 *
 * The caveat is one clause, not a paragraph. A first version explained the
 * confound in four lines on the screen, which is the same mistake the picker
 * notes made: true, and more than anybody reading a phone wants. Why week 1 is
 * the honest week belongs here; the screen needs the number and the warning
 * not to over-read the trend.
 */
function renderChalk(rows, mean) {
  ...
  const drift = rows.length > 1
    ? ` Later weeks read flatter — ${esc((last.top * 100).toFixed(0))}% by week
        ${esc(last.week)} — because a team can only be taken once.`
    : '';
  return `
    <p class="note">
      ...
      the cleanest read on how chalky this pool is.${drift}
    </p>`;
}
