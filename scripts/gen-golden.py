...
This is the oracle half of the parity contract. The Python in survivor-picker/
is the *definition* of what these algorithms do; this script records that
...
sys.path.insert(0, str(ROOT / "survivor-picker"))
...
def main() -> None:
    ...
    fmt_path = ROOT / "test/fmt-cases.json"
    if check:
        if not fmt_path.exists() or fmt_path.read_text() != fmt_cases():
            stale.append("test/fmt-cases.json")
    else:
        fmt_path.write_text(fmt_cases())
        print("  test/fmt-cases.json")
