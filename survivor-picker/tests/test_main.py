...
from main import _fetch_pipeline_games, _remaining_pool, compute_held_back_teams
...
class TestRemainingPool:
    def test_excludes_used_teams(self):
        remaining = _remaining_pool(["KC", "BUF"])
    ...
    def test_no_used_teams_returns_full_list(self):
        assert _remaining_pool([]) == NFL_TEAMS

    def test_preserves_nfl_teams_order(self):
        remaining = _remaining_pool(["KC"])
...
class TestFetchPipelineGames:
    def test_fetches_current_and_lookahead_weeks(self):
        ...
        current_week, current_games, all_games = _fetch_pipeline_games(client, week=None, lookahead_weeks=2)
    ...
    def test_explicit_week_is_used_as_current_week(self):
        ...
        current_week, current_games, all_games = _fetch_pipeline_games(client, week=4, lookahead_weeks=1)
        assert current_week == 4
        assert client.requested_weeks == [4, 5]

    def test_no_current_week_games_returns_empty(self):
        client = FakeESPNClient({})
        current_week, current_games, all_games = _fetch_pipeline_games(client, week=None, lookahead_weeks=3)
